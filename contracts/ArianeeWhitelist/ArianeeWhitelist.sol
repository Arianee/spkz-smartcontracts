...
Abilitable{
   ...
   */
  mapping(uint256=> mapping(address=>bool)) internal optOutAddressPerOwner;
   ...
   */
  event WhitelistedAddressAdded(uint256 indexed _tokenId, address indexed _address);
  ...
  function addWhitelistedAddress(uint256 _tokenId, address _address) external hasAbilities(ABILITY_ADD_WHITELIST) {
      ...
      whitelistCounter[_tokenId] = whitelistCounter[_tokenId] + 1;
  ...
  function addBlacklistedAddress(address _sender, uint256 _tokenId, bool _activate) external {
      optOutAddressPerOwner[_tokenId][_sender] = _activate;
   ...
   */
  function isAuthorized(uint256 _tokenId, address _sender) external view returns(bool) {
      return (isWhitelisted(_tokenId, _sender) && !isBlacklisted(_sender, _tokenId));
  }
   ...
   */
  function isBlacklisted(address _sender, uint256 _tokenId) public view returns(bool _isBlacklisted) {
      _isBlacklisted = optOutAddressPerOwner[_tokenId][_sender];
  }
